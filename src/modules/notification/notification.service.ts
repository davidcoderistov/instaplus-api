...
export class NotificationService implements INotificationService {
    ...
    public async findUserHasUnseenNotifications(userId: string): Promise<UserHasUnseenNotifications> {
        ...
        const hasUnseenNotifications = await this._notificationRepository.findUserHasUnseenNotifications(userId, userNotificationHistory ? userNotificationHistory.seenAt as unknown as Date : new Date(0))
